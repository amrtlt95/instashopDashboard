...
def loopProducts(driver,special=""):
    ...
    for i in range(len(allProducts)):
        ...
        if (special=="Outers" and not isOuter(allMultiplierQuantities[i])) \
                ...
                or(special == "Gallons" and not CheckIfItemIsGallonWater(allProductsTypes[i])): # this checks if the current item is not outer , normal diapers or golden rice
            continue
        time.sleep(3)
        allInfoButton[i].click()
        waitForFakeLoading(driver)
        time.sleep(3)
        #statusButton = WebDriverWait(driver,60).until(
            #EC.element_to_be_clickable(By.XPATH,"/html/body/div[1]/div[9]/div[13]/div/div/div/div/div[2]/div/md-radio-group/md-radio-button[15]")
...
def CheckIfItemIsGallonWater(productType):
    return productType.text == "water dispenser bottles"
...
def handelReport(cat2dArray,currentItem,isScanned):
...
#find_elements(By.TAG_NAME,"li") # this is a list of categories buttons
 # this is used in the next loops to confirm if the category cell is found , to not break the loop
  # ignore it now
  #initial variables
productsSearchTextBox = WebDriverWait(driver,60).until(
    EC.element_to_be_clickable((By.XPATH,"/html/body/div[1]/div[9]/div[146]/div[6]/div[1]/div/div[1]/div[1]/div/input")
))
...
arrOfAll = [[  # auto
    ...
    [
        ...
        ["Green Tea", "Green Teas", "Black Tea","Black Teas"],
    ...
    [
        ...
        ["Cotton Buds & Pads", "Cotton Pads", "Cotton Buds", "Makeup Pads"],
        ["Makeup Removers"],
    ...
    [
        ...
        ["Cotton Buds & Pads", "Cotton Buds", "Cotton Pads"],
        ["Sun Care", "Sun Block"],
    ...
    [
        ["Healthy & Organic"],  # pharmacy Report
    ]
    ,
    ...
    [  # back To School
        ["Stationery"],
         ["Tape", "Eraser", "Pencils", "Pens", "Markers", "notebooks", "Coloring Notebooks",
         "Sketches", "Sticky Notes", "Correction Pen", "Scissors", "Copy Papers", "School Bags", "Story Books",
         "Pencil Sharpener"]
    ]
    ,
    ...
    [
        ["Vegan & Vegetarian"] #pharmacy report
...
for element in categories:
    ...
    if catNameInDashboard == catNameInArrOfAll:
        ...
        waitForLoading(driver)
        time.sleep(2)
        categoryButton = element.find_elements(By.CLASS_NAME, "btn-link")
        catReportResult = ""
        catReportResult = ""
        healthyCounter = 0
        emptyCatCounter = 0
        ...
        for i in range(1,len(arrOfAll[arrOfAllCatCounter])):
            ...
            for j in range(len(arrOfAll[arrOfAllCatCounter][i])):
                ...
                elif currentItem == "Outers" or currentItem == "Normal Diapers" or currentItem == "Golden" or currentItem == "Gallons" :
                    time.sleep(4)
                ...
                if Handle:
                    handelReport(arrOfAll[arrOfAllCatCounter],currentItem,isCurrentItemScanned)
